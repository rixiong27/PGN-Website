...
function VotingPage() {
  ...
  const vote = (pnmId: number, score: number) => { setScores((current) => ({ ...current, [`${activeRound?.id}-${pnmId}`]: score })); if (activeRound) castVote.mutate({ id: activeRound.id, data: { pnmId, score } }, { onSuccess: () => { toast.show('Vote saved.'); void queryClient.invalidateQueries({ queryKey: getGetVotingRoundQueryKey(activeRound.id) }); void queryClient.invalidateQueries({ queryKey: getGetDashboardQueryKey() }); } }); };
  return <><PageHeader eyebrow="Workspace / Voting" title="Voting rounds" subtitle="Make a clear call, then keep moving." action={isAdmin ? <button className="btn btn-primary" onClick={() => setShowCreate((value) => !value)} data-testid="button-create-round"><Plus size={15} /> Open round</button> : null} />{showCreate ? <form className="card toolbar" onSubmit={submitRound} style={{ display: 'grid', gridTemplateColumns: '1fr 180px 1.6fr auto', gap: 12, marginBottom: 18 }}><div className="field"><label className="field-label" htmlFor="round-name">Round name</label><input id="round-name" required className="input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Second round conversations" data-testid="input-round-name" /></div><div className="field"><label className="field-label" htmlFor="round-deadline">Deadline</label><input id="round-deadline" className="input" type="date" value={form.deadline} onChange={(e) => setForm({ ...form, deadline: e.target.value })} data-testid="input-round-deadline" /></div><div className="field"><label className="field-label">Include PNMs</label><select className="select" multiple value={selectedPnms.map(String)} onChange={(e) => setSelectedPnms(Array.from(e.target.selectedOptions).map((option) => Number(option.value)))} style={{ height: 38 }} data-testid="select-round-pnms">{pnms?.map((pnm) => <option key={pnm.id} value={pnm.id}>{pnm.firstName} {pnm.lastName}</option>)}</select></div><button className="btn btn-primary" type="submit" disabled={createRound.isPending || !selectedPnms.length} data-testid="button-save-round">Open</button></form> : null}<div className="card round-card"><div className="section-head"><div><div className="section-title">Open now <span style={{ color: 'hsl(var(--primary))', fontFamily: 'var(--app-font-mono)' }}>{openRounds.length}</span></div><div className="page-subtitle" style={{ fontSize: 12, marginTop: 3 }}>Your votes are private to you until the round closes.</div></div></div>{isLoading ? <LoadingBlock rows={4} /> : isError ? <ErrorState onRetry={() => refetch()} /> : !openRounds.length ? <EmptyState title="No open voting rounds" copy="When a round opens, your PNMs will appear here." icon={ClipboardCheck} /> : openRounds.map((round) => <div key={round.id} onClick={() => setSelected(round.id)} style={{ cursor: 'pointer' }}><div className="round-card-head"><div><div className="round-name">{round.name}</div><div className="round-detail">{round.pnmIds.length} PNMs · {round.voteCount} votes cast{round.deadline ? ` · closes ${formatDate(round.deadline)}` : ''}</div></div><div className="row-actions"><StatusBadge value={round.status} />{isAdmin ? <button className="btn btn-secondary btn-sm" onClick={(event) => { event.stopPropagation(); closeRound.mutate({ id: round.id }, { onSuccess: () => { void queryClient.invalidateQueries({ queryKey: getListVotingRoundsQueryKey() }); toast.show('Voting round closed.'); } }); }} data-testid={`button-close-round-${round.id}`}>Close round</button> : null}</div></div>{activeRound?.id === round.id ? <div>{round.pnmIds.map((pnmId) => { const pnm = pnms?.find((item) => item.id === pnmId); const result = detail?.results?.find((item) => item.pnmId === pnmId); const key = `${round.id}-${pnmId}`; return <div className="pnm-vote-row" key={pnmId} data-testid={`row-vote-${pnmId}`}><div className="person"><div className="avatar">{initials(pnm ? `${pnm.firstName} ${pnm.lastName}` : 'PNM')}</div><div><div className="person-name">{pnm ? `${pnm.firstName} ${pnm.lastName}` : result?.pnmName ?? `PNM #${pnmId}`}</div><div className="person-sub">{pnm?.major ?? 'Candidate context'}{isAdmin && result ? ` · ${result.voteCount} votes / avg ${result.average?.toFixed(1) ?? '—'}` : ''}</div></div></div><div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>{isAdmin && result?.average ? <span className="score">{result.average.toFixed(1)}</span> : null}<div className="vote-scale">{[1,2,3,4,5].map((score) => <button className={`vote-btn ${scores[key] === score ? 'selected' : ''}`} key={score} onClick={() => vote(pnmId, score)} disabled={castVote.isPending} data-testid={`button-vote-${pnmId}-${score}`}>{score}</button>)}</div></div></div>; })}</div> : null}</div>)}</div><div className="card round-card"><div className="section-head"><div><div className="section-title">Closed history</div><div className="page-subtitle" style={{ fontSize: 12, marginTop: 3 }}>Past decisions stay available for reference.</div></div></div>{closedRounds.length ? closedRounds.map((round) => <div className="round-item" key={round.id}><div><div className="round-name">{round.name}</div><div className="round-detail">{round.pnmIds.length} PNMs · {round.voteCount} total votes · closed {formatDate(round.closedAt)}</div></div><StatusBadge value={round.status} /></div>) : <EmptyState title="No closed rounds" copy="Closed rounds will appear here after decisions are final." icon={Archive} />}</div><Toast message={toast.message} /></>;
}
