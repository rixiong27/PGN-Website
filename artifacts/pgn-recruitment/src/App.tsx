...
function ActionError({ title, message }: { title: string; message: string }) {
  return <div className="error-state" role="alert" data-testid="action-error"><strong>{title}</strong> <span>{message}</span></div>;
}
...
function SignUpPage() {
  ...
  return (
    <div className="auth-shell">
      <div className="auth-visual"><Logo dark /><div className="auth-statement"><div className="eyebrow" style={{ color: 'hsl(0 72% 68%)' }}>Step 1 of 2 · Create account</div><h1>Create your account.</h1><p>Use any email address you can verify. You’ll join the VT PGN chapter with its code in the next step.</p></div><div className="auth-footer">VERIFIED EMAIL REQUIRED</div></div>
      <div className="auth-panel">
  ...
  const submit = (event: React.FormEvent) => {
    ...
    joinChapter.mutate({ data: { code: code.trim() } }, {
      onSuccess: (member) => {
       ...
       onError: (error) => setError(apiErrorMessage(error, 'That chapter code is not valid. Check it and try again.')),
    });
  };
  return (
    <div className="auth-shell">
...
function Roster() {
  ...
  const params = useMemo(() => {
    const statusKey = Object.entries(pipelineLabels).find(([, label]) => label === status)?.[0] ?? status;
    return { ...(search ? { search } : {}), ...(status ? { status: statusKey as PipelineStatus } : {}), sort, archived: false };
  }, [search, status, sort]);
  ...
  const submit = (event: React.FormEvent) => {
    ...
    create.mutate({ data }, {
      onSuccess: (created) => {
        ...
        setShowAdd(false);
        setFormError('');
        setForm({ firstName: '', lastName: '', email: '', major: '', year: 'Freshman', semester: 'Fall 2026' });
        const visibleListKey = getListPnmsQueryKey({ archived: false, sort });
        queryClient.setQueryData<Pnm[]>(visibleListKey, (current) => {
          const withoutCreated = (current ?? []).filter((pnm) => pnm.id !== created.id);
          return [...withoutCreated, created];
        });
        void queryClient.invalidateQueries({ queryKey: getListPnmsQueryKey() });
...
function Profile() {
  ...
  const removePnm = () => {
    setRemoveError('');
    if (!window.confirm(`Delete ${pnm.firstName} ${pnm.lastName} permanently? This removes the PNM record and its notes.`)) return;
    remove.mutate({ id: pnmId }, {
      onSuccess: () => {
        void queryClient.invalidateQueries({ queryKey: getListPnmsQueryKey() });
        void queryClient.invalidateQueries({ queryKey: getGetDashboardQueryKey() });
        void queryClient.invalidateQueries({ queryKey: getListActivityQueryKey() });
...
function Members() {
  ...
  const activeMemberCount = !isLoading && !isError && data
    ? data.filter((member) => member.status === 'active' && (member.role === 'member' || member.role === 'admin' || member.role === 'super_admin')).length
    : null;
